...
\begin{figure*}[t] % 
\centering
\includegraphics[width=\textwidth]{Piplines.pdf} 
...
To capture non-linear relationships and feature interactions that may be missed by the Cox model, we implement a \textbf{Random Survival Forest} \citep{fantazzini2009random} with 1,000 trees using log-rank splitting. This ensemble method provides both predictive accuracy and variable importance rankings through minimal depth analysis. This research also employs \textbf{DySurv} \citep{mesinovic2024dysurv}, a deep learning architecture originally developed for clinical applications. DySurv uses a conditional variational autoencoder to learn latent representations of posts, outputting discrete-time hazard predictions across 72 one-hour intervals.

Training data spans 2018 to mid-2024, with subsequent 6-month periods for development and testing. Hyperparameter optimization employs nested 5×2 cross-validation on the training set. We evaluate performance using Harrell's C-Index (concordance), Integrated Brier Score (calibration over time), and Expected Calibration Error (see Appendix B for detailed specifications).
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{combined_calibration_hazard_no_title.pdf}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{combined_balance_hte_no_title.pdf}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{offline_evaluation_diagnostics_no_title.pdf}
