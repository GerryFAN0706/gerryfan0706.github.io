...
\newcommand{\best}[1]{\cellcolor{Lavender!35}\textbf{#1}}
\newcommand{\second}[1]{\cellcolor{Honeydew!45}#1}
% -------------------------------------------------------
...
\end{equation}
This accounting is intentionally conservative: we do not exploit tighter composition via RDP or correlations between concept features.
...
\begin{equation}
  Y_{u,j} =
  \begin{cases}
    X_{u,j}, & \text{w.p. } p_b = \dfrac{e^{\epsilon_b}}{1+e^{\epsilon_b}},\\[0.6ex]
...
\end{equation}
user-level LDP by basic composition. For our default hyperparameters ($m{=}2^{18}$, $r{=}64$, $\epsilon_b{=}0.1$, $K{=}12$, $R{=}4$, $\epsilon_c{=}0.05$) we obtain $\epsilon_{\text{hash}}{=}6.4$ and $\epsilon_{\text{concept}}{\le}2.4$, hence a total user-level budget of
$\epsilon_{\text{user}} \le 8.8$ per user (or per evaluation window). In Sec.~\ref{sec:exp} we therefore report both the hashed-feature budget $\epsilon_{\text{hash}}$ and the total $\epsilon_{\text{user}}$ when interpreting privacy--utility trade-offs.
...
\begin{equation}
  \epsilon_c \;\lesssim\;
  \ln\!\Bigl(1 + \tfrac{e^{\epsilon_{\text{user}}}-1}{\sqrt{n}}
  \cdot \sqrt{2n\ln(1/\delta)} + \tfrac{(e^{\epsilon_{\text{user}}}-1)^2}{n}\Bigr).
\end{equation}
...
Higher is better for AUPRC/AUROC/F1; lower for Brier/ECE. All values are means over 5 seeds (two decimal places). Best and second-best per column are highlighted.}
...
\vspace{0.5em}
\begin{flushleft}
\footnotesize \emph{Notes.} Best per column is \best{highlighted}; second-best is \second{shaded}. The embedding baseline serves as an upper-bound reference (dense, semantic, no privacy). Our full method balances privacy (hashed channel $\epsilon_{\text{hash}}{=}6.4$, total $\epsilon_{\text{user}}{\le}8.8$), interpretability (12 concepts), and competitive utility.
\end{flushleft}
\end{table*}
...
This work is still subject to several important limitations. First, teacher bias and cross-cultural validation. Concept labels are produced by frozen LLMs using prompt-based rubrics; this introduces potential cultural and rubric biases. While the concept set draws on constructs with cross-cultural support (Sec.~\ref{sec:concept}), we lack human-annotated concept data from domain experts to quantify clinician agreement, particularly for culture-sensitive constructs such as self-blame, help-seeking, and all-or-nothing thinking. Future work should incorporate expert review across languages, especially for high-stakes concepts such as hopelessness \cite{gilardi2023chatgptannotators,tan2024llmannotationSurvey}.
...
Third, longitudinal and multi-episode composition. Our $\epsilon_{\text{user}}$ guarantee applies per screening episode. Under basic composition, $E$ independent episodes accumulate to $E \cdot \epsilon_{\text{user}}$; for example, quarterly screening over one year ($E{=}4$) yields a cumulative budget of $4 \times 8.8 = 35.2$. Tighter accounting via R\'enyi DP (RDP) reduces this: converting each episode to $(\alpha, \rho)$-RDP with $\rho = \frac{1}{\alpha-1}\ln\!\bigl(\alpha p_b^{\alpha}(1{-}p_b)^{1-\alpha} + (1{-}p_b)^{\alpha}p_b^{1-\alpha}\bigr)$ and summing $E$ episodes gives a cumulative $(\epsilon, \delta)$-DP bound that is substantially tighter than $E\epsilon_{\text{user}}$ at moderate $E$ \cite{duchi2013localprivacy,kairouz2015composition}. In practice, three mitigations limit cumulative exposure: (i) capping screening frequency (e.g., once per quarter); (ii) salt rotation across episodes, which makes reports from different episodes formally unlinkable; and (iii) optional shuffle amplification (Sec.~\ref{sec:ldp}), which reduces the effective per-episode central $\epsilon$ when a shuffler is available. Designing mechanisms with explicit multi-episode user-level guarantees remains an important direction \cite{acharya2023userldp}.
Fourth, mechanism design and utility. Our LDP design uses simple subsampled randomized response for hashed features and $G$-ary randomized response for concept summaries; counting mechanisms such as OLH/OUE or Hadamard Response \cite{wang2017ldp,acharya2019hr} could provide better privacy--utility trade-offs, especially for high-dimensional sparse vectors, and are a natural extension. Integrating a shuffle layer \cite{balle2019shuffle,erlingsson2019shuffle,bittau2017prochlo} could further reduce the effective central $\epsilon$ (Sec.~\ref{sec:ldp}), particularly for tighter local budgets, at the cost of an additional trust assumption.
